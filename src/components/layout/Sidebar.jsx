...
const sidebarMenus = [
  ...
  { name: "Sales Dashboard", route: "/sales-dashboard", icon: SalesDashboardMenuIcon },
];

const sidebarBottomMenus = [
  { name: "Settings", route: "/settings", icon: SettingsMenuIcon },
  {
    name: "Belk",
...
const Sidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  return (
    <div className={`sidebar fixed z-50 bg-[#242424] top-0 left-0 h-full transition-transform duration-300 w-[270px] lg:w-[341px] ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
          ...
          {sidebarMenus?.map((item, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleMenuClick(item?.route)} className={`flex gap-3 p-4 w-full items-center rounded-[8px] ${isActive ? "bg-[#D0CAC2]" : ""}`}>
                ...
                <p className={`text-[18px] font-[Poppins] ${isActive ? "text-[#000]" : "text-[#FFF]"}`}>{item?.name}</p>
              </button>
            );
          })}
        </div>
            ...
            return (
              <div key={idx} className="w-full relative">
                <button
                  ...
                  className={`w-full p-4 flex gap-3 items-center rounded-[8px] ${isActive ? "bg-[#D0CAC2]" : ""}`}
                >
                  <Icon color={isActive ? "#000" : "#FFF"} />
                  <div className="flex justify-between w-full items-center">
                    <p className={`text-[18px] font-[Poppins] ${isActive ? "text-[#000]" : "text-[#FFF]"}`}>{item?.name}</p>
                    {isBelk && (
                      <SidebarLeftArrowIcon className={`transform transition-transform ${openSubMenu ? "rotate-90" : ""}`} color={isActive ? "#000" : "#FFF"} />
                    )}
                  </div>
                </button>
                {isBelk && openSubMenu && (
                  <div className="absolute bottom-[60px] left-0 w-full bg-white shadow-lg rounded-[8px] z-50">
                    {item?.subMenus?.map((sub, subIdx) => (
                      <button key={subIdx} onClick={() => handleMenuClick(sub?.route)} className={`w-full text-left px-4 py-3 font-[Poppins] text-[16px] ${pathname === sub?.route ? "text-blue-700" : "text-[#000]"}`}>
                        {sub?.name}
                      </button>
                    ))}
                  </div>
